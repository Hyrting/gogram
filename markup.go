...
func (r *ReplyMarkup) Inline(rows ...Row) {
	...
	for i, row := range rows {
		...
		for j, btn := range row {
			...
			if btn == nil {
				panic(fmt.Sprintf(
					"gogram: button row %d column %d is not an inline button",
					i, j))
			}
...
func (r *ReplyMarkup) Reply(rows ...Row) {
	...
	for i, row := range rows {
		...
		for j, btn := range row {
			...
			if btn == nil {
				panic(fmt.Sprintf(
					"gogram: button row %d column %d is not a reply button",
					i, j))
			}
			keys = append(keys, *btn)
		}
...
//
// To pass the pointers to bool use a special gogram.Flag function,
