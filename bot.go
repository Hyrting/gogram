...
func NewBot(pref Settings) (*Bot, error) {
	...
	bot := &Bot{
		...
		client:           client,
		useGoroutinePool: pref.useGoroutinePool,
	}
...
type Bot struct {
	...
	useGoroutinePool bool       // New flag to toggle goroutine pool
	pool             *ants.Pool // New goroutine pool
}
...
// Example:
//
//	b.Handle("/start", func (c gogram.Context) error {
//		return c.Reply("Hello!")
//	})
//
//	b.Handle(&inlineButton, func (c gogram.Context) error {
//		return c.Respond(&gogram.CallbackResponse{Text: "Hello!"})
//	})
//
// Middleware usage:
//
...
func (b *Bot) Handle(endpoint interface{}, h HandlerFunc, m ...MiddlewareFunc) {
	...
	if end == "" {
		panic("gogram: unsupported endpoint")
	}
...
func (b *Bot) Trigger(endpoint interface{}, c Context) error {
	...
	if end == "" {
		return fmt.Errorf("gogram: unsupported endpoint")
	}

	handler, ok := b.handlers[end]
	if !ok {
		return fmt.Errorf("gogram: no handler found for given endpoint")
	}
...
func (b *Bot) Start() {
	if b.Poller == nil {
		panic("gogram: can't start without a poller")
	}
	...
	// Initialize the goroutine pool if enabled
	if b.useGoroutinePool {
		var err error
		// Initialize pool with -1 (which makes it auto-expand as needed)
		b.pool, err = ants.NewPool(-1)
		if err != nil {
			panic(err)
		}
		defer b.pool.Release() // Make sure the pool is released when done
	}
	...
	for {
		...
		case upd := <-b.Updates:
			if b.useGoroutinePool {
				// Use the goroutine pool to process the update
				...
				// Directly process update without pool
				b.ProcessUpdate(upd)
			}
...
// SendAfter sends a message to the specified recipient after a given duration (d).
// It accepts the recipient, the message (what), and any additional send options.
// It returns a pointer to the Message sent and an error if occurred.
func (b *Bot) SendAfter(d time.Duration, to Recipient, what interface{}, opts ...interface{}) (*Message, error) {
	var sentMessage *Message

	// Schedule sending the message after the specified duration
	time.AfterFunc(d, func() {
		// Attempt to send the message using the Send method
		var err error
		sentMessage, err = b.Send(to, what, opts...)
		if err != nil {
			// Handle any error that occurs during the send operation
			// (Logging or invoking an error handler can be added here)
		}
	})

	// Return the sent message (or nil if not sent yet)
	return sentMessage, nil // Note: sentMessage will be nil until it's sent.
}

// SendAfterSignal waits for a message from the provided channel (Chan)
// and sends it to the specified recipient when a message is received.
// It returns the sent Message and an error if occurred.
func (b *Bot) SendAfterSignal(Chan chan string, to Recipient, opts ...interface{}) (*Message, error) {
	// Wait for the channel to send a message
	message := <-Chan // Block until a message is received

	// Attempt to send the message using the Send method
	sentMessage, err := b.Send(to, message, opts...)
	if err != nil {
		// Handle any error that occurs during the send operation
		// (Logging or invoking an error handler can be added here)
		return nil, err
...
func (b *Bot) SendPaid(to Recipient, stars int, a PaidAlbum, opts ...interface{}) (*Message, error) {
	...
	for i, x := range a {
		...
		if repr == "" {
			return nil, fmt.Errorf("gogram: paid media entry #%d does not exist", i)
		}
...
// From all existing options, it only supports gogram.Silent.
func (b *Bot) SendAlbum(to Recipient, a Album, opts ...interface{}) ([]Message, error) {
	...
	for i, x := range a {
		repr := x.MediaFile().process(strconv.Itoa(i), files)
		if repr == "" {
			return nil, fmt.Errorf("gogram: album entry #%d does not exist", i)
		}
...
//	b.Edit(m, "new <b>text</b>", gogram.ModeHTML)
//	b.Edit(m, &gogram.ReplyMarkup{...})
//	b.Edit(m, &gogram.Photo{File: ...})
//	b.Edit(m, gogram.Location{42.1337, 69.4242})
...
// Use cases:
//
//	b.EditMedia(m, &gogram.Photo{File: gogram.FromDisk("chicken.jpg")})
//	b.EditMedia(m, &gogram.Video{File: gogram.FromURL("http://video.mp4")})
func (b *Bot) EditMedia(msg Editable, media Inputtable, opts ...interface{}) (*Message, error) {
	...
	case file.OnDisk() || file.FileReader != nil:
		...
		files[s] = *file
	default:
		return nil, fmt.Errorf("gogram: cannot edit media, it does not exist")
	}
...
func (b *Bot) File(file *File) (io.ReadCloser, error) {
	...
	if resp.StatusCode != http.StatusOK {
		resp.Body.Close()
		return nil, fmt.Errorf("gogram: expected status 200 but got %s", resp.Status)
	}
